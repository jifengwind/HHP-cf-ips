    ...
    return new Promise((resolve) => {
        ...
        https.get(url, (res) => {
            ...
            res.on('end', () => {
                ...
                // 匹配表格中的每一行
                const rowPattern = /<tr>\s*<td>(\d+)<\/td>\s*<td>([^<]+)<\/td>\s*<td>([^<]+)<\/td>\s*<td>([^<]+)<\/td>\s*<td>([^<]+)<\/td>\s*<td>([^<]+)<\/td>/g;
                
                let match;
                while ((match = rowPattern.exec(html)) !== null) {
                    const lineType = match[2].trim();      // 线路类型（电信/联通/移动/多线）
                    ...
                    const speed = parseFloat(speedText.replace('mb/s', '')) || 0;
                    ...
                    // 优先使用页面标注的线路类型，否则用经验识别
                    const isp = (lineType !== 'IPV6') ? lineType : detectISP(ip);
...
async function main() {
    ...
    finalIPs.slice(0, 10).forEach((c, i) => {
        ...
        console.log(`   ${i + 1}. ${c.ip}:${PORT}#${c.isp} (延迟: ${c.latency}ms, 速度: ${c.speed}mb/s, ${scoreInfo})`);
    });
